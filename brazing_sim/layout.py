...
class ShallowULayout:
    ...
    # The compact single-tier deck stays behind S3's 350 mm table
    # footprint.  Their nearest edges retain a real 40 mm service aisle.
    fin_magazine_xy: tuple[float, float] = (0.32, -0.320)
    fin_pickup_x: float = 0.32
    fin_pickup_surface_z_m: float = 0.25
    fin_row_spacing_m: float = 0.075
    dense_fin_row_spacing_m: float = 0.031
    fin_single_tier_span_m: float = 0.19
    ...
    def raw_fin_position(
        ...
        *,
        ...
        Orders with at most four fins retain the verified 75 mm centre pitch,
        leaving wide clearance for the gripper.  Five/six-fin orders use all
        of the compact deck, while C and denser products use the dense pitch
        and shrink only as required to fit the 190 mm usable Table1 span.
        Every blank remains on the same physical pickup surface; a seventh fin
        is never hidden on an upper shelf.
        """
